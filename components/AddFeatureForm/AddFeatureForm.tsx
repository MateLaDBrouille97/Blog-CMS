...
// import InputDow from "../InputDrop/Input";
import InputDow from "../InputDow/InputDow";
...
type Props = {
  ...
  handleFeatureChange2: (event: ChangeEvent<HTMLSelectElement>) => void;
  featureFormData: {
    name: string;
    description: string;
    finishDate: string;
...
const AddFeatureForm: FC<Props> = (props) => {
  ...
  return (
    ...
    >
      <button
        type="button"
        className="absolute top-3 right-2.5 text-gray-400 hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center bg-gray-200"
      >
        <MdCancel onClick={toggleAddFeatureForm} className="text-3xl" />
      </button>

      <div className="px-6 py-6 lg:px-8">
        <h3 className="mb-4 text-xl font-medium">Add Feature Board</h3>
