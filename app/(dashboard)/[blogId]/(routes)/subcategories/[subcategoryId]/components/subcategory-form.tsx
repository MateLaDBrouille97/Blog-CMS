...
import { BillboardBlog, CategoryBlog } from "@prisma/client"
...
const formSchema = z.object({
  ...
  categoryBlogId: z.string().min(1),
});

type SubCategoryFormValues = z.infer<typeof formSchema>

interface SubCategoryFormProps {
  initialData: CategoryBlog | null;
...
export const SubCategoryForm: React.FC<SubCategoryFormProps> = ({
  ...
  const onSubmit = async (data: SubCategoryFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/${params.blogId}/subcategories/${params.subcategoryId}`, data);
      } else {
        await axios.post(`/api/${params.blogId}/subcategories`, data);
      }
      router.refresh();
      router.push(`/${params.blogId}/subcategories`);
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.blogId}/subcategories/${params.subcategoryId}`);
      router.refresh();
      router.push(`/${params.blogId}/subcategories`);
