...
export const getGraphCount = async (blogId: string): Promise<GraphData[]> => {
  ...
  for (const month in monthlyCount2) {
    for (const year in monthlyCount2) {
      const key = `${year}-${month}`;
      graphData[parseInt(key)].totalChapter = monthlyCount2[parseInt(key)];
    }
  }

  for (const month in monthlyCount2) {
    for (const year in monthlyCountArticles2) {
      const key = `${year}-${month}`;
      graphData[parseInt(key)].totalArticle = monthlyCountArticles2[parseInt(key)];
    }
  }
